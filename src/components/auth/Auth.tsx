...
export default function Auth() {
  ...
  const handleGithubLogin = async () => {
    try {
      ...
      // Note: Navigation will be handled by the auth state change in AuthContext
    } catch (err) {
      setError(
        err instanceof Error
          ? err.message
          : 'Ошибка входа через GitHub. Пожалуйста, попробуйте снова.'
      )
    }
  }
        ...
        <div className="space-y-6">
          ...
          >
            <span className="absolute left-0 inset-y-0 flex items-center pl-3">
              <Github className="h-5 w-5 text-gray-300 group-hover:text-gray-400" />
            </span>
            Войти через GitHub
          </button>
